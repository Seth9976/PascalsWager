-- chunkname: @f:/unity/pascalswager_steam1/assets/LuaScript/Logic/AI/Behaviour/HoverKeep.lua

require("LuaScript/Lib/func")

local BT = require("LuaScript/Lib/behaviourtree")

HoverKeep = class("HoverKeep")
HoverKeep.static.MaxTime = 1.5
HoverKeep.static.MinTime = 1
HoverKeep.static.MaxDistance = 12
HoverKeep.static.LockTime = 1

function HoverKeep:initialize(instance, waitTimeRatio, breakDistance)
	...
	self.hoverKeepNode = self:create()
end

function HoverKeep:create()
	local btnode = BT.SequenceNode:create({
		BT.IsBehaviourStateNode:create(self.instance, GameAI.BehaviourState.HoverKeep),
		BT.ConditionNode:create(function()
			...
			if self.waitTime > HoverKeep.LockTime or self.instance:IsFreeMoveEnable() == false then
		...
		BT.SelectorNode:create({
			BT.SequenceNode:create({
				...
				BT.CommonNode:create(function()
					...
					self.changeStateTime = self.waitTimeRatio * (MathHelper.getRandom(HoverKeep.MinTime * 10, HoverKeep.MaxTime * 10, self.instance:GetAnimalId()) * 0.1)
					self.waitTime = 0
				end),
				BT.ConditionWaitNode:create(function()
					...
					if GameAI.IsAnimalWithinDistance(self.instance, target:GetPosByWorldPos(), self.breakDistance) == true then
						self.instance:GetBrain().behaviourState = GameAI.BehaviourState.Attack

						return true
					end

					if GameAI.IsAnimalWithinDistance(self.instance, target:GetPosByWorldPos(), HoverKeep.MaxDistance) == false then
						self.instance:GetBrain().behaviourState = GameAI.BehaviourState.Attack
