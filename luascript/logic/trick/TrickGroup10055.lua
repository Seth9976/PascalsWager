-- chunkname: @f:/unity/pascalswager_steam1/assets/LuaScript/Logic/Trick/TrickGroup10055.lua

require("LuaScript/Logic/Trick/TrickGroupBase")
require("GameData/TrickDefine/TrickGroup10055_Define")

TrickGroup10055 = class("TrickGroup10055", TrickGroupBase)

function TrickGroup10055:initialize(scene)
...
function TrickGroup10055:OpenAuto()
	TrickGroupBase.OpenAuto(self)

	self.gameController = self.sceneController:GetGameController()

	self:SetDelayClear(1, true)
	self:OpenDelay(TrickGroup10055_Define.DelayTime)
end

function TrickGroup10055:Open(pos, rot)
	...
	if self.trickActive ~= nil then
		...
		if self.swordList ~= nil then
			for k, v in pairs(self.swordList) do
				local shellId = self.controllerShell:CreateShell(v.transform, 0, 2, TrickGroup10055_Define.ShellId, ControllerShell.ShellShape.Capsule)
...
function TrickGroup10055:Update()
