-- chunkname: @F:/Unity/PascalsWager_steam1/Assets/LuaFramework/Lua/3rd/pbc/parser.lua
...
patterns.ENUM = Ct(Cg("enum", "type") * blanks * Cg(typename, "name") * blank0 * "{" * blank0 * Cg(lpeg.Cf(Ct("") * enum_item^1, insert), "value") * "}" * blank0)

local prefix_field = P("required") * Cc("LABEL_REQUIRED") + P("optional") * Cc("LABEL_OPTIONAL") + P("repeated") * Cc("LABEL_REPEATED")
...
local message_field = lpeg.Cf(Ct(Cg(prefix_field, "label") * blanks * Cg(typename, "type_name") * blanks * Cg(name, "name") * blank0 * "=" * blank0 * Cg(id, "number")) * blank0 * options, setoption) * blank0 * ";" * blank0
local extensions = Ct(Cg("extensions", "type") * blanks * Cg(id, "start") * blanks * "to" * blanks * Cg(id, "end") * blank0 * ";" * blank0)

patterns.EXTEND = Ct(Cg("extend", "type") * blanks * Cg(typename, "name") * blank0 * "{" * blank0 * Cg(Ct(message_field^1), "extension") * "}" * blank0)
patterns.MESSAGE = P({
	Ct(Cg("message", "type") * blanks * Cg(typename, "name") * blank0 * "{" * blank0 * Cg(Ct((message_field + patterns.ENUM + extensions + patterns.EXTEND + V(1))^0), "items") * "}" * blank0)
})
patterns.OPTION = Ct(Cg("option", "type") * blanks * Cg(name, "name") * blank0 * "=" * blank0 * Cg(value, "value")) * blank0 * ";" * blank0
...
function parser.register(fileset, path)
	...
	local pbencode = pb.encode("google.protobuf.FileDescriptorSet", {
		file = files
	})
